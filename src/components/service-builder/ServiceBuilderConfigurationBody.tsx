...
import { PAINT_CONDITIONS, VEHICLE_SIZES, WAX_OPTIONS } from '@/constants/serviceBuilderCatalog'
import {
...
type Props = {
  ...
  /** Barre estimate + Next step en bas (feuille carte). Sur la page Service Builder, laisser à false (bannière sticky globale). */
  showEstimateBar?: boolean
  /** Offset scroll pour ancrer la première section sous la bannière sticky (page seulement). */
  ...
  return (
    <div className={cn('flex min-h-0 flex-col', showEstimateBar && 'gap-0')}>
      ...
      >
        <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-[#424245]">This helps us tailor the service to your vehicle</p>
            {importedVehicle ? (
      ...
      <article className="border-t border-black/10 pt-8 transition md:pt-10">
        ...
        <p className="mb-6 text-sm text-[#424245]">Add extra gloss and depth with a premium wax layer</p>
        <ServiceBuilderChoiceRail>
          {WAX_OPTIONS.map((wax) => {
